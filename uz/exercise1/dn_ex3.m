...
eagle_mask = eagle_gray < threshold; % glih obratno z tresholdom, ali open namesto close
eagle_se = strel('diamond', 5); % operacija open in close sta si nasprotna.
% % open image, negate the result
eagle_mask = imerode(imdilate(eagle_mask, eagle_se), eagle_se); % close
% eagle_mask = not(imdilate(imerode(eagle_mask, eagle_se), eagle_se)); % open
